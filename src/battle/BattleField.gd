...
# Trap & Support slots
var active_trap_card: CardData = null
var active_support_card: CardData = null
...
var pending_ultimate_card: CardData = null
...
func _setup_battle() -> void:
	player_data = GameManager.active_hero.duplicate_data()
	enemy_data = GameManager.active_enemy_data.duplicate_data() if GameManager.active_enemy_data else CharacterData.new()
	...
	# Prepare draw pile
	draw_pile = GameManager.player_deck_ids.duplicate()
	draw_pile.shuffle()
	discard_pile.clear()
...
func _spawn_card_in_hand(c_data: CardData) -> void:
...
func _on_card_played(c_data: CardData, card_node: Control) -> void:
	...
	discard_pile.append(c_data.id)
	
	# Execute Card Logic
	if c_data.card_type == CardData.CardType.ULTIMATE:
...
func _apply_card_effect(c_data: CardData, qte_multiplier: float = 1.0) -> void:
	var combo_mult = combo_meter.get_multiplier()
	var final_multiplier = combo_mult * qte_multiplier
	
	match c_data.card_type:
		CardData.CardType.TAIJUTSU, CardData.CardType.NINJUTSU:
		...
		CardData.CardType.SUPPORT:
			...
			if c_data.status_effects.size() > 0:
			...
			if c_data.base_damage > 0:
				_damage_character(enemy_data, enemy_visual, c_data.base_damage)
				
		CardData.CardType.ULTIMATE:
			var ult_dmg = int(c_data.base_damage * final_multiplier)
			player_visual.play_attack_animation(enemy_visual, c_data.animation_key)
			_damage_character(enemy_data, enemy_visual, ult_dmg)
			combo_meter.add_combo(c_data.combo_points, c_data.hit_count)
...
func _on_end_turn_pressed() -> void:
	...
	for c in hand_cards:
		discard_pile.append(c.card_data.id)
		c.queue_free()
	hand_cards.clear()
